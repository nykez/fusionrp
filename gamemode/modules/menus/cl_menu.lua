...
Fusion.menu.list = Fusion.menu.list or {}

-- Adds a new menu to the list of drawn menus.
function Fusion.menu.add(options, position, onRemove)
	...
	for k, v in ipairs(Fusion.menu.list) do
		...
		for k2, v2 in SortedPairs(v.options) do
			...
			if (inside and mY >= y and mY <= (y + 28)) then
				...
				surface.SetDrawColor(ColorAlpha(Color(32, 32, 32), v.alpha + math.cos(RealTime() * 8) * 40))
				surface.DrawRect(startX, y, width, 28)
			end

			-- Draw the button's text.
			Fusion.util.drawText(k2, startX + 4, y, ColorAlpha(color_white, v.alpha), nil, nil, "TargetID")
...
function Fusion.menu.getActiveMenu()
	local mX, mY = ScrW() * 0.5, ScrH() * 0.5
	local position2 = LocalPlayer():GetPos()
...
function Fusion.menu.onButtonPressed(menu, callback)
	table.remove(Fusion.menu.list, menu)

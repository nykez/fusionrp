...
Fusion.config.itemFormat = "<font=TargetID>%s</font>\n<font=TargetID>%s</font>"
...
function PANEL:PaintOver(w, h)
	local itemTable = Fusion.item.instances[self.itemID]
...
vgui.Register("fusionItemIcon", PANEL, "SpawnIcon")
...
PANEL = {}
	...
	function PANEL:setInventory(inventory)
		if (inventory.slots) then
			if (IsValid(Fusion.gui.inv1) and Fusion.gui.inv1.childPanels and inventory != LocalPlayer():getChar():getInv()) then
			...
			for x, items in pairs(inventory.slots) do
				for y, data in pairs(items) do
					...
					local item = Fusion.item.instances[data.id]
	...
	function PANEL:onTransfer(oldX, oldY, x, y, oldInventory, noSend)
		local inventory = Fusion.item.inventories[oldInventory.invID]
		local inventory2 = Fusion.item.inventories[self.invID]
		local item
		...
		if (inventory) then
			...
			if (hook.Run("CanItemBeTransfered", item, Fusion.item.inventories[oldInventory.invID], Fusion.item.inventories[self.invID]) == false) then
	...
	function PANEL:addIcon(model, x, y, w, h, skin)
		...
		if (self.slots[x] and self.slots[x][y]) then
			local panel = self:Add("fusionItemIcon")
			...
			panel.OnMouseReleased = function(this, code)
				if (code == MOUSE_LEFT and Fusion.item.held == this) then
...
vgui.Register("fusionInventory", PANEL, "DFrame")

concommand.Add("gui_inventory", function()
	Fusion.gui.inv1 = vgui.Create("fusionInventory")
	Fusion.gui.inv1.childPanels = {}
	...
	Fusion.gui.inv1:Center()
end)



hook.Add("PostRenderVGUI", "fusionInvHelper", function()
	local pnl = Fusion.gui.inv1

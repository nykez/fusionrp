...
function META:setOwner(owner, fullUpdate)
	...
	if (SERVER) then
		...
		mysql:RawQuery("UPDATE fusion_inventories SET _charID = "..owner.." WHERE _invID = "..self:getID())
	end
	...
	if (onlyMain != true) then
		...
		if (#bags > 0) then
			for _, invID in ipairs(bags) do
				local bagInv = Fusion.item.inventories[invID]
...
function META:remove(id, noReplication, noDelete)
	...
	if (SERVER and !noReplication) then
		...
		if (!noDelete) then
			...
			mysql:RawQuery("DELETE FROM fusion_items WHERE _itemID = "..id)
			Fusion.item.instances[id] = nil
		end
	end
		...
		for k2, v2 in pairs(v) do
			if (v2 and !items[v2.id]) then
				...
				if (isBag and isBag != self:getID() and onlyMain != true) then
					local bagInv = Fusion.item.inventories[isBag]
	...
	quantity = quantity or 1
		...
		if (quantity > 0) then
			...
			if (type(uniqueID) == "number") then
				local item = Fusion.item.instances[uniqueID]
				...
				if (item) then
					...
					if (hook.Run("CanItemBeTransfered", item, Fusion.item.inventories[0], targetInv) == false) then
					...
					if (x and y) then
						...
						if (!self.noSave) then
							mysql:RawQuery("UPDATE fusion_items SET _invID = "..targetInv:getID()..", _x = "..x..", _y = "..y.." WHERE _itemID = "..item.id)
						end
				...
				if (hook.Run("CanItemBeTransfered", itemTable, Fusion.item.inventories[0], targetInv) == false) then
					return false, "notAllowed"
				end
				...
				if (x and y) then
					...
					Fusion.item.instance(targetInv:getID(), uniqueID, data, x, y, function(item)
						item.gridX = x
						item.gridY = y

...
if (SERVER) then
	...
	function ENT:setItem(itemID)
		local itemTable = Fusion.item.instances[itemID]
	...
	function ENT:OnRemove()
		if (!Fusion.shuttingDown and !self.fusionIsSafe and self.fusionItemID) then
			local itemTable = Fusion.item.instances[self.fusionItemID]
			...
			if (itemTable) then
				...
				mysql:RawQuery("DELETE FROM fusion_items WHERE _itemID = "..self.fusionItemID)
			end
		end
	end
...
function ENT:getItemTable()
	return Fusion.item.list[self:getItemID()]
end
